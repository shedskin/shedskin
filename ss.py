...
class globalInfo: # XXX add comments
    def __init__(self):
        ...
        self.cpp_keywords.update(['stdin', 'stdout', 'stderr', 'std', 'abstract', 'open', 'read', 'write', 'close', 'chdir', 'getcwd']) # XXX
        self.list_types = {}
...
def check_redef(node): # XXX to modvisitor
    if not mv.module.builtin:
        for whatsit in [mv.classes, mv.ext_classes, mv.funcs, mv.ext_funcs]:
            if node.name in whatsit:
                error("function/class redefinition is not supported ('%s')" % node.name, node)
    ...
    def visitModule(self, node, declare=False):
        ...
        if declare: 
            ...
            for child in node.node.getChildNodes():
                if isinstance(child, From): # and not gx.modules[child.modname].builtin: 
                    skip = True
                    for (name, pseudonym) in child.names:
                        if name == '*':
                            name = child.modname
                            if '.' in name: name = name[name.rfind('.')+1:] # XXX yeck!
                            for func in gx.modules[child.modname].funcs.values():
                                if func.cp: # and name != 'math':
                                    print >>self.out, 'using __'+name+'__::'+self.cpp_name(func.ident)+';';
                            for var in gx.modules[child.modname].mv.globals.values():
                                if not var.invisible and not var.imported and not var.name.startswith('__'):
                                    print >>self.out, 'using __'+name+'__::'+self.cpp_name(var.name)+';';
                            for cl in gx.modules[child.modname].classes:
                                print >>self.out, 'using __'+name+'__::'+cl+';';

                            continue

                        modname = child.modname
                        if '.' in modname: modname = modname[modname.rfind('.')+1:] # XXX yeck!
                        print >>self.out, 'using __'+modname+'__::'+self.nokeywords(name)+';'
...
#                    break
    ...
    def visitAugAssign(self, node, func=None):
        ...
        if isinstance(node.node, Subscript):
            self.start()
            if set([t[0] for t in self.mergeinh[node.node.expr]]) == set([defclass('dict')]) and node.op == '+=':
    ...
    def visitCallFunc(self, node, func=None): 
        ...
        if ident in ['max','min'] and len(node.args) > 3:
    ...
    def expandspecialchars(self, value):
        ...
        for i in range(len(value)):
            if value[i] in replace: value[i] = '\\'+replace[value[i]]
            elif value[i] < ' ' or value[i] > '~': value[i] = '\\'+oct(ord(value[i]))
...
def types_merge_copybyvalue(node, cf, argument=False):
    if (node, cf.dcpa, cf.cpa) not in gx.cnode: return [] # XXX ?
    argnode = gx.cnode[node, cf.dcpa, cf.cpa] 
    types = argnode.types()

    # --- merge copy-by-value types, as well as non-reflowed types in case of data cpa
    result = set()
    for t in types: 
        if t[0].ident in ['int_', 'float_', 'none', 'class_', 'str_']:
            result.add((t[0], 0))
        else:
            result.add(t)
...
def template_parameters():
    ...
    for cl in gx.allclasses: # (first do class template vars, as function depend on them) # XXX recursion!
             ...
             if cl == defclass('dict') and 'unit' in cl.vars and 'value' in cl.vars:
...
def template_detect(var, parent):
    ...
    if (var.name == 'unit' and var.parent.ident in ['list','tuple','set','frozenset','__iter']) or (var.name in ['unit', 'value'] and var.parent.ident == 'dict') or (var.name in ['first', 'second'] and var.parent.ident == 'tuple2'):
...
def main():
    ...
    print '*** SHED SKIN Python-to-C++ Compiler 0.0.25 ***'
    print 'Copyright 2005-2007 Mark Dufour; License GNU GPL version 3 (See LICENSE)'
