...
cnf = [
    l.strip().split() for l in open(argv[1]) if l[0] not in "c0%\n"
]  # [list(list(str))]
clauses = [
...
def solve_rec():  # la_mods: [list(int)], var: [int], prop_mods: [list(int)], choice: [int]*
    ...
    if not -1 in fixedt[1:]:  # [int]
        print(
            "v",
            " ".join([str((2 * fixedt[i] - 1) * i) for i in range(1, nrofvars + 1)]),
        )  # [list(str)]
        return 1  # [int]
...
def propagate(
    lit, mods, failed_literal=0
):  # lit_truth: [int], current: [int], unfixed: [int]*, mods: [list(int)], clause: [list(int)], lit: [int]*, length: [int], failed_literal: [int]
...
def lookahead(
    mods,
):  # mods: [list(int)], i: [int], u: [list(int)], var: [int], part: [list(int)]
    ...
    while global_mask != 0:  # [int]
        ...
        for i in range(32):  # [int]
            ...
            for var in part:  # [int]
                if fixedt[var] == -1 and not lookahead_variable(var, mods, dif):
                    return 0  # [int]
...
def lookahead_variable(
    var, mods, dif
):  # mods: [list(int)], dif: [list(int)], choice: [int]*, var: [int], prop: [int]
    ...
    for choice in [var, -var]:  # [int]
        ...
        if not prop:  # [int]
            ...
            some_failure = 1  # [int]
            if not propagate(-choice, mods, 1):
                return 0  # [int]
            break
