...
def mySuper(self, material=None, name=None):
...
#{
    ...
    if False:
#}
      mySuper(self, name=name, material=material)
...
#}
  ...
  # Sphere hits are always treated as bidirectional unless inside.
  def hitTest(self, ray, bidirectional=False, inside=False, best=None):
#{
...
#}
    ...
    if (D >= 0):
      ...
      if inside:
        ...
        distance = -B - sqrtD
#TODO: Something's funny here.  Why doesn't this work?  It should only ever
#      improve things, and only for hits coming from inside the sphere.  The
#      rest of the time it should be harmless, and yet it isn't.  The assert
#      fails, and even without that, the image is wrong, with or without the
#      inverted flag.
#        if distance < 0:
#          distance = -B + sqrtD
#          if distance >= 0:
#            # We must be inside the sphere, bouncing off some other object.
