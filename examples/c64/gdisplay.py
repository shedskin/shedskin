...
#		box.pack_start(controls, False, False)
	...
	def repaint_pixmap(self):
		...
		color = self.colors[self.border_color]
		...
		GC.set_foreground(self.colors[self.border_color])
		...
		GC.set_foreground(self.colors[self.background_color_0])
		...
		for row in range(24): # FIXME 25
			for column in range(40): # FIXME configurable
				...
				GC.set_foreground(self.colors[color if code < 128 else self.background_color_0]) # FIXME
