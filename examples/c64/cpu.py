...
class CPU(object):
    ...
    def store_value(self, addressing_mode, value, size = 1):
        ...
        if addressing_mode == "Z":
            ...
            # FIXME unsigned?
            self.MMU.write_memory((self.consume_unsigned_operand(1) + (self.read_register("Y")) & 0xFF), value, size)
        elif addressing_mode == "Z+X":
            # FIXME unsigned?
            self.MMU.write_memory((self.consume_unsigned_operand(1) + (self.read_register("X")) & 0xFF), value, size)
        elif addressing_mode == "ABS":
            self.MMU.write_memory(self.consume_unsigned_operand(2), value, size)
        elif addressing_mode == "ABS+Y":
            # FIXME unsigned.
            self.MMU.write_memory((self.consume_unsigned_operand(2) + self.read_register("Y")) & 0xFFFF, value, size)
        elif addressing_mode == "ABS+X":
            # FIXME unsigned.
            self.MMU.write_memory((self.consume_unsigned_operand(2) + self.read_register("X")) & 0xFFFF, value, size)
        elif addressing_mode == "IND+X":
            base = self.consume_unsigned_operand(1)
            # FIXME signed?
            offset = (self.read_register("X"))
    ...
    def load_value_advancing(self, addressing_mode):
        ...
        return  self.read_register("A") if addressing_mode == "A" else \
            ...
            self.MMU.read_zero_page((self.consume_unsigned_operand(1) + self.read_register("X")) & 0xFF) if addressing_mode == "Z+X" else \
            self.MMU.read_zero_page((self.consume_unsigned_operand(1) + self.read_register("Y")) & 0xFF) if addressing_mode == "Z+Y" else \
            self.MMU.read_memory(self.consume_unsigned_operand(2)) if addressing_mode == "ABS" else \
            self.MMU.read_memory((self.consume_unsigned_operand(2) + self.read_register("X")) & 0xFFFF) if addressing_mode == "ABS+X" else \
            self.MMU.read_memory((self.consume_unsigned_operand(2) + self.read_register("Y")) & 0xFFFF) if addressing_mode == "ABS+Y" else \
            self.MMU.read_memory(self.MMU.read_memory((self.consume_unsigned_operand(1) + self.read_register("X")) & 0xFF, 2)) if addressing_mode == "IND+X" else \
            self.MMU.read_memory(self.read_zero_page_memory(self.consume_unsigned_operand(1), 2) + self.read_register("Y")) if addressing_mode == "IND+Y" else \
...
#            print("N not dead")
    ...
    def TAY(self, opcode = 0xA8):
        """ transfer A to Y """
        self.write_register("Y", self.update_flags_by_number(self.read_register("A")))

    def TYA(self, opcode = 0x98):
        """ transfer Y to A """
        self.write_register("A", self.update_flags_by_number(self.read_register("Y")))

    def TAX(self, opcode = 0xAA):
        """ transfer A to X """
        self.write_register("X", self.update_flags_by_number(self.read_register("A")))

    def TSX(self, opcode = 0xBA):
        """ transfer SP to X """
        value = self.update_flags_by_number(self.read_register("SP"))
        if (value & 128) != 0:
            self.flags.add("N")
        else:
            self.flags.discard("N")
    ...
    def TXA(self, opcode = 0x8A):
        """ transfer X to A """
        self.write_register("A", self.update_flags_by_number(self.read_register("X")))
    ...
    def BNE(self, opcode):
        ...
        if "Z" not in self.flags:
            #print("OFFSET", offset)
            self.write_register("PC", (self.read_register("PC") + offset) & 0xFFFF)
