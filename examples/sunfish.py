...
'''
Sunfish: a compact Python chess engine

https://github.com/thomasahle/sunfish

copyright Thomas Ahle, license GPL3
'''
...
#                            and j not in (self.ep, self.kp, self.kp-1, self.kp+1): break
                    if p == 'P' and (d == N or d == N+N) and q != '.': break
                    if p == 'P' and d == N+N and (i < A1+N or self.board[i+N] != '.'): break
...
class Searcher:
    ...
    def moves(self, depth, root, pos, gamma):
        ...
        val_move = [(pos.value(move), move) for move in pos.gen_moves()]
        for val, move in sorted(val_move, reverse=True):
    ...
    def bound(self, pos, gamma, depth, root=True):
        ...
        best = -MATE_UPPER
        for move, score in self.moves(depth, root, pos, gamma):
            best = max(best, score)
            if best >= gamma:
...
def main():
    ...
    while True:
        ...
        while move not in hist[-1].gen_moves():
            match = re.match('([a-h][1-8])'*2, 'd2d4') # XXX just testing one move here #input('Your move: '))
            if match:
...
#                break
        ...
        break  # XXX added to time performance
