...
class SurfacePoint(object):
    ...
    def get_emission(self, to_position, out_direction, is_solid_angle):
        ...
        assert (isinstance(cos_area, float) and isinstance(max(distance2, 1e-6) if is_solid_angle else 1.0, float))
        solid_angle = cos_area / max(distance2, 1e-6) if is_solid_angle else 1.0 ## truediv
    ...
    def get_reflection(self, in_direction, in_radiance, out_direction):
        ...
        return ZERO if (in_dot < 0.0) ^ (out_dot < 0.0) else in_radiance.mul(self.triangle_ref.reflectivity) * (abs(in_dot) / pi) ## truediv

    def get_next_direction(self, in_direction):
        reflectivity_mean = self.triangle_ref.reflectivity.dot(ONE) / 3.0 ## truediv
        if random() < reflectivity_mean:
            color = self.triangle_ref.reflectivity * (1.0 / reflectivity_mean) ## truediv
            _2pr1 = pi * 2.0 * random()
            sr2 = sqrt(random())
            x = (cos(_2pr1) * sr2)
            y = (sin(_2pr1) * sr2)
