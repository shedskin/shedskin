...
str *__moddict(str *v, dict<str *, str *> *d) {
    /* autogenerated: get ordered list of keys (names); remove keys */
    str *const_5 = new str("%(");
    str *const_6 = new str(")");
    str *const_7 = new str("(");
    str *const_8 = new str("");
    ...
    while (1) {
        ...
        names->append(v->__slice__(3, (i+2), j, 0));
    }

    FOR_IN(name,(new set<str *>(names)),1)
    ...
    while((k = __fmtpos(fmt)) != -1) {
        char c = fmt->unit[k];
        str *arg = d->__getitem__(names->__getitem__(argcount++));
