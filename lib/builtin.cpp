...
int str::__cased_outside(char a, char b) {
    int any_cased = 0;
    int l = unit.size();
    for(int i=0; i<l; i++) 
    {
        ...
        if((c >= 'a' && c <= 'z') || (c >= 'A' && c <= 'Z')) any_cased = 1;
    }
    return any_cased;
}

int str::islower() {
    return __cased_outside('A', 'Z');
}
int str::isupper() {
    return __cased_outside('a', 'z');
}
...
int str::__cmp__(pyobj *p) {
    str *b = (str *)p;
    if( unit < b->unit ) return -1;
    else if( unit > b->unit ) return 1;
    return 0;
}
...
str *str::__add__(str *b) {
    return new str(unit+b->unit); /* XXX optimize */
}
