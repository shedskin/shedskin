...
def copy_method(self, cl, name, declare):
    ...
    if not declare:
        ...
        for var in cl.vars.values():
            if not var.invisible and var in config.getgx().merged_inh and config.getgx().merged_inh[var]:
...
def deepcopy_classes(classes):
    ...
    while changed:
        ...
        for cl in classes.copy():
            for var in cl.vars.values():
                if var in config.getgx().merged_inh:
                    newcl = set(t[0] for t in config.getgx().merged_inh[var] if isinstance(t[0], class_) and not t[0].mv.module.builtin)
                    if newcl - classes:
                        changed = True
...
def determine_classes():  # XXX modeling..?
    if 'copy' not in config.getgx().modules:
        return
    func = config.getgx().modules['copy'].mv.funcs['copy']
    var = func.vars[func.formals[0]]
    classes = set(t[0] for t in config.getgx().merged_inh[var] if isinstance(t[0], class_) and not t[0].mv.module.builtin)
    for cl in classes:
        cl.has_copy = True
    func = config.getgx().modules['copy'].mv.funcs['deepcopy']
    var = func.vars[func.formals[0]]
    classes = set(t[0] for t in config.getgx().merged_inh[var] if isinstance(t[0], class_) and not t[0].mv.module.builtin)
