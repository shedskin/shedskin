...
def annotate(gx):
    ...
    for module in gx.modules.values():
        ...
        merge = dict(
            (k, v)
            for k, v in gx.merged_inh.items()
            if (k, 0, 0) in gx.cnode and infer.inode(gx, k).mv == mv
        )
        ...
        for expr in merge:
            if isinstance(expr, (ast.Num, ast.Str, ast.Name)):
                paste(
                    ...
                    expr,
                    typestr.nodetypestr(
                        gx, expr, infer.inode(gx, expr).parent, False, mv=mv
                    ),
                    mv,
                )
        ...
        for cl in mv.classes.values():
            labels = [
                var.name + ": " + typestr.nodetypestr(gx, var, cl, False, mv=mv)
                for var in cl.vars.values()
                if var in merge and merge[var] and not var.name.startswith("__")
            ]
            if labels:
        ...
        for func in funcs:
            ...
            labels = [
                var.name + ": " + typestr.nodetypestr(gx, var, func, False, mv=mv)
                for var in vars
            ...
            if isinstance(callfunc.node, ast.Attribute):
                if not callfunc.node.__class__.__name__.startswith(
                    ...
                    paste(
                        ...
                        callfunc.node.expr,
                        typestr.nodetypestr(
                            gx, callfunc, infer.inode(gx, callfunc).parent, False, mv=mv
                        ),
                        mv,
                    )
            else:
        ...
        for expr in merge:
            if isinstance(expr, ast.ListComp):
                ...
                paste(
                    ...
                    expr,
                    typestr.nodetypestr(
                        gx, expr.value, infer.inode(gx, expr).parent, False, mv=mv
                    ),
                    mv,
                )
                ...
                paste(
                    ...
                    ", ".join(
                        typestr.nodetypestr(
                            gx, child, infer.inode(gx, child).parent, False, mv=mv
                        )
        ...
        for expr in merge:
            if isinstance(expr, ast.Assign):
                ...
                paste(
                    ...
                    ", ".join(
                        typestr.nodetypestr(
                            gx, r, infer.inode(gx, r).parent, False, mv=mv
                        )
                        for (l, r) in pairs
                    ),
                    mv,
                ...
                paste(
                    ...
                    expr,
                    typestr.nodetypestr(
                        gx, expr.expr, infer.inode(gx, expr).parent, False, mv=mv
                    ),
                    mv,
                )
