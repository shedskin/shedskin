...
def shape_subsumes(a: str, b: str) -> bool:
    """Is `a` the same shape as `b` except that some parts are still empty?

    Two contours of one class whose renderings differ only where one has an
    empty type variable are not really distinguishable yet: nothing has
    reached that variable, so there is no evidence the two differ. Splitting
    to separate them therefore resolves nothing.

    This is deliberately positional, comparing the two renderings piece by
    piece, so a contour that is genuinely different anywhere -- not merely
    unpopulated -- is never subsumed.
    """
    if a == b:
        return True
    if "{}" not in a:
        return False
    # a is b with one or more type variables still empty: match a against b
    # positionally, letting each empty slot in a stand for any single type set
    pattern = re.escape(a).replace(re.escape("{}"), r"\{[^{}]*\}")
    return re.fullmatch(pattern, b) is not None
    ...
    for ident, rendered in shapes.items():
        ...
        for i in range(len(rendered)):
            for j in range(i + 1, len(rendered)):
                ...
                if a == b or shape_subsumes(a, b) or shape_subsumes(b, a):
                    return True
    return False
