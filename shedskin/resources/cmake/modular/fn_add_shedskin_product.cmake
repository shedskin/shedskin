...
function(add_shedskin_product)
    ...
    if(BUILD_EXECUTABLE)
        ...
        if(IS_NESTED)
            add_custom_command(OUTPUT ${translated_files}
                COMMAND shedskin --nomakefile -o ${PROJECT_EXE_DIR}/${parentpath} ${opts} translate "${SHEDSKIN_MAIN_MODULE}"
            ...
            add_custom_command(OUTPUT ${translated_files}
                COMMAND shedskin --nomakefile -o ${PROJECT_EXE_DIR} ${opts} translate "${main_py}"
                WORKING_DIRECTORY ${CMAKE_CURRENT_SOURCE_DIR}
                DEPENDS "${main_py}"
                COMMENT "translating ${main_py} to exe"
                VERBATIM
    ...
    if(BUILD_EXTENSION)
        ...
        if(IS_NESTED)
            add_custom_command(OUTPUT ${translated_files}
                COMMAND shedskin --nomakefile -o ${PROJECT_EXT_DIR}/${parentpath} -e ${opts} translate "${SHEDSKIN_MAIN_MODULE}"
                WORKING_DIRECTORY ${CMAKE_CURRENT_SOURCE_DIR}
                DEPENDS "${SHEDSKIN_MAIN_MODULE}"
            ...
            add_custom_command(OUTPUT ${translated_files}
                COMMAND shedskin --nomakefile -o ${PROJECT_EXT_DIR} -e ${opts} translate "${main_py}"
                WORKING_DIRECTORY ${CMAKE_CURRENT_SOURCE_DIR}
                DEPENDS "${main_py}"
                COMMENT "translating ${main_py} to ext"
                VERBATIM
