...
class generateVisitor(ASTVisitor):
    ...
    def connector(self, node, func):
        if singletype(node, module):
            return '::'
        complextype = set([(defclass('complex'),0)])
        if inode(node).types().intersection(complextype):
            return '.'
        return '->'
    ...
    def visitMod(self, node, func=None):
        ...
        for n in nodes:
            if [clname for clname in ('complex', 'float_', 'int_', 'bool_') if (defclass(clname), 0) in self.mergeinh[n]]:
