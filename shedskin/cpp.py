...
class GenerateVisitor(ast_utils.BaseNodeVisitor):
    ...
    def visit_Call(
        ...
        if self.library_func(funcs, "array", "array", "__init__"):
            if (
                ...
                or node.args[0].value not in "bBuwhHiIlLqQfd"
            ):
                error.error(
    ...
    def genexpr_class(self, node: ast.ListComp, declare: bool) -> None:
        ...
        if declare:
            ...
            for a, b in args:
                self.output(a + b + ";")
            self.output("typedef size_t for_in_loop;")
            self.output("int __last_yield;\n")
            self.output(func2 + " myresult;\n")
            self.output(func1 + ";")
            self.output(func2 + " __get_next();")
            self.output(func2 + " __get_next_awesome();")
            self.output("inline size_t for_in_init();")
            self.output("inline bool for_in_has_next(size_t i);")
            self.output("inline " + func2 + " for_in_next(size_t &i);")
            self.deindent()
            self.output("};\n")
        else:
            ...
            self.deindent()
            self.output("}\n")
            self.output("inline size_t " + lcfunc.ident + "::for_in_init() {")
            self.output("    __stop_iteration = false;")
            self.output("    return 0;")
            self.output("}\n")
            self.output("inline bool " + lcfunc.ident + "::for_in_has_next(size_t i) {")
            self.output("    myresult = __get_next_awesome();")
            self.output("    return !__stop_iteration;")
            self.output("}\n")
            self.output("inline " + func2 + " " + lcfunc.ident + "::for_in_next(size_t &i) {")
            self.output("    return myresult;")
            self.output("}\n")
