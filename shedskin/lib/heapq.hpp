...
template<class T> nheapiter<T, Cmp> *nlargest(long int /* key is None */, __ss_int n, pyiter<T> *iterable) {
    return new nheapiter<T, Cmp>(n, iterable);
}

template<class T, class B> nheapiter<T, Cmp> *nlargest(B (*key)(T), __ss_int n, pyiter<T> *iterable) {
    return new nheapiter<T, Cmp>(n, iterable);
}
