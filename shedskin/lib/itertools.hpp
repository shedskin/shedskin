...
template<class T, class U> tuple2<T, U> *izipiter<T, U>::next() {
    ...
    try  {
        tuple->first = this->iters[0]->next();
        tuple->second = this->iter2->next();
    } catch (StopIteration *) {
        this->exhausted = true;
        throw;
    }
...
template<class T> class izip_longestiter;
template<class T> inline izip_longestiter<T> *izip_longest(T fillvalue, int iterable_count, pyiter<T> *iterable, ...);

template<class T> class izip_longestiter : public __iter<tuple2<T, T> *> {
public:
    int exhausted;
    ...
    friend izip_longestiter<T> *izip_longest<T>(T fillvalue, int iterable_count, pyiter<T> *iterable, ...);
};

template<class T> inline izip_longestiter<T>::izip_longestiter() {
    this->exhausted = 0;
}
template<class T> inline izip_longestiter<T>::izip_longestiter(T fillvalue, pyiter<T> *iterable) {
    this->exhausted = 0;
    this->push_iter(iterable);
    this->fillvalue = fillvalue;
}

template<class T> void izip_longestiter<T>::push_iter(pyiter<T> *iterable) {
    this->iters.push_back(iterable->__iter__());
    this->exhausted_iter.push_back(0);
}

template<class T> tuple2<T, T> *izip_longestiter<T>::next() {
  ...
  template<class T> inline izip_longestiter<void*> *izip_longest(T /* fillvalue */, int /* iterable_count */) {
    return new izip_longestiter<void*>();
}
template<class T> inline izip_longestiter<T> *izip_longest(T fillvalue, int iterable_count, pyiter<T> *iterable, ...) {
  izip_longestiter<T> *iter = new izip_longestiter<T>(fillvalue, iterable);
...
template<class T> class productiter;
template<class T> inline productiter<T> *product(int repeat, int iterable_count, pyiter<T> *iterable, ...);

template<class T> class productiter : public __iter<tuple2<T, T> *> {
public:
    bool exhausted;
    int highest_exhausted;
    ...
    friend productiter<T> *product<T>(int repeat, int iterable_count, pyiter<T> *iterable, ...);
};

template<class T> inline productiter<T>::productiter() {
    this->exhausted = false;
    this->highest_exhausted = 0;
}

template<class T> void productiter<T>::push_iter(pyiter<T> *iterable) {
...
template<class T> tuple2<T, T> *productiter<T>::next() {
    if (this->exhausted) {
        throw new StopIteration();
    }
...
inline productiter<void*> *product(int /* repeat */, int /* iterable_count */) {
    return new productiter<void*>();
}
template<class T> inline productiter<T> *product(int repeat, int iterable_count, pyiter<T> *iterable, ...) {
  productiter<T> *iter = new productiter<T>();
...
template<class T> tuple2<T, T> *permutationsiter<T>::next() {
    ...
    for (; ; ) {
        ...
        if (cycle) {
            ...
            std::swap(this->indices[this->current], this->indices[this->len - cycle]);
            tuple2<T, T> *tuple = new tuple2<T, T>;
            for (int i = 0; i < this->r; ++i) {
