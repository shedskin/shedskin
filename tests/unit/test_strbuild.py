...
class TestNonFunctionParent:
    """cpp.py's visitor passes whatever parent it has, not always a Function"""

    def test_class_parent_is_refused(self, gx):
        # a loop in a class body is visited with the class as parent; sb_begin
        # must not hand that to the analysis (it has no .node, .vars, etc.)
        from shedskin import cpp

        source = "class C:\n for i in r:\n  out += 'x'\n"
        loop = ast.parse(source).body[0].body[0]
        mv = gx.modules["builtin"].mv
        cls = python.StaticClass(
            python.Class(
                gx, ast.parse("class C: pass").body[0], mv, gx.modules["builtin"]
            ),
            mv,
        )
        visitor = cpp.GenerateVisitor.__new__(cpp.GenerateVisitor)
        visitor.sb_loops = {}
        assert cpp.GenerateVisitor.sb_begin(visitor, loop, cls) == []

    def test_none_parent_is_refused(self, gx):
        from shedskin import cpp

        loop = ast.parse("for i in r:\n out += 'x'\n").body[0]
        visitor = cpp.GenerateVisitor.__new__(cpp.GenerateVisitor)
        visitor.sb_loops = {}
        assert cpp.GenerateVisitor.sb_begin(visitor, loop, None) == []
